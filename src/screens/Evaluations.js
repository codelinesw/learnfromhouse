   ...
   IsJsonString(str) {
	...
	_renderItems_(item,index){
		...
		if(item === "" || item === "undefined" || item === null || item === undefined){
			return(<tr key={index+1}><td></td><td></td><td><div className="container_circle_preloader d-flex position-relative justify-content-center align-items-center mb-5" style={{width:'75px',height:'50px',top:'15px',left:'240px'}}>
			...
			if(Object.values(item)[0] === "empty"){
				return <tr key={index+1}><td><p>No hay tareas creadas aun</p></td></tr>;
			}else{
				return(
					<tr key={index+1}>
						...
						<td style={{fontsize:'12px',paddingTop:'15px'}}>
							...
							}} className="btn btn-outline-dark p-1 pl-3 pr-3"><i className="fas fa-eye"></i></Link></div>
						}
						</td>
					</tr>
				);
